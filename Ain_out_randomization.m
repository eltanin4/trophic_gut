function [m2b, b2m] = Ain_out_randomization(b_real, i_all_filt, j_all_filt, v_all_filt)
...
% i_all_filt, j_all_filt, v_all_filt is a triplet of the network edges.
    ...
    in_degree=sum(sign(m2b),1)';  % calculate in degrees of all bacterial species
    [i2,j2,v2]=find(m2b);
    ...
    %b2m=ones(size(i1));
    b2m=rand(size(i1));   % randomizing byproduct production
    b2m=sparse(i_all_filt(i1),j_all_filt(i1),b2m,2244,2244);
    out_degree=full(sum(sign(b2m), 1)'); % calculate out degrees of all bacterial species
    [i2,j2,v2]=find(b2m);
