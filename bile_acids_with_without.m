...
%% Non-linear optimization for one individual 3 without bile acids
clear all;
...
x_full = zeros(2244,1);
x_full(i_intake_diet) = x;
%[a,b] = sort(x_full(i_intake_diet),'descend');  
%x_full(i_intake_diet(b(5:end))) = 0;   % keep only top 4 metabolites
...
% i_nutrient_intake containins labels of diets we want to optimize over
i_list  = unique(j_all_filt(find((v_all_filt==7) .* (i_all_filt~=2170)))); 
i_nutrient_intake = [i_list([3:(end-2),end]); i_sugar_diet([3,7]); i_bile_acids(:)];
