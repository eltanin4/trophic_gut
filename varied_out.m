...
predicted_b_a_without_variation = predicted_b_a;

pred_metabolome_without_variation = c_layer(i_common,4);
...
% A_in, A_out matrix [c2b_real, bp_out] and [mu_matrix, ctot_matrix] is calculated
[m2b, b2m] = Ain_out_varied_out(b_real, i_all_filt, j_all_filt, v_all_filt);
[m2b_total, m2m_total, m2b_layer] = mu(f, m2b, b2m, numLayer_max);
...
%%
figure;
loglog(predicted_b_a_without_variation, predicted_b_a_with_variation,'ko')
hold on;
plot([1e-6, 1],[1e-6, 1],'k-')
xlabel('Equal lambda out, metagenomics')
ylabel('Random lambda out, metagenomics')

figure;
loglog(pred_metabolome_with_variation / sum(pred_metabolome_with_variation), pred_metabolome_without_variation / sum(pred_metabolome_without_variation),'ko')
hold on;
plot([1e-6, 1],[1e-6, 1],'k-')
xlabel('Equal lambda out, metabolome')
ylabel('Random lambda out, metabolome')
